# # Programmer: Carla Casanova Suárez
#
#  ----------------------
# |     INSTRUCTIONS     |
#  ----------------------
#
# This is a python tool that, given a FASTA or FASTQ file, preprocess all the sequences contained in it. 
# The tool has to be able to perform, reverse-complement, trim, and adaptor-removal. 
# Depending on the command line options used, the tool will perform a different preprocessing operation.
#
# The following criteria has been followed while programming:
#   1. The tool assumes that the input file is properly formatted and contains no errors. 
#   2. The tool will check that all the arguments required have been provided by the user.
#   3. The tool has to be able to work with both FASTA/FASTQ file formats. 
#   4. The output of the execution has to be in the same format as the input.
#
# There are three different methods available for preprocessing the FASTA/FASTQ:
#   a) Reverse-complement: 
#      If the user specifies the option --operation rc, the tool has to compute the reverse complement of each sequence 
#      in the input dataset and write them into the corresponding FASTQ/FASTA output file. 
#   b) Trim: 
#      If the user specifies the option --operation trim, the tool must perform a sequence trim on the input dataset. 
#      For that, the user must specify to the tool the length of the trim by using both or one of this options: --trim-right or --trim-left.
#   c) Adaptor removal:
#      If the user specifies the option --operation adaptor-removal, the tool must search for an adaptor sequenced (provided by the user) 
#      at the beginning of each sequence in the input dataset. So, the argument --adaptor should be also specified.
#      If found, the input sequence must be trimmed and the adaptor sequence removed (i.e., trimmed from the left of the sequence). 
#
# At the end, the tool must show a brief summary with the total number of reads processed (i.e., sequences) 
# and the total number of bases processed (together with the percentage of ‘A’s, ‘C’s, ‘G’s, ‘T’s, and ‘N’s found in the input file)
